...
* 服务 **reliable** 到底意味着什么：也就是在出错时仍能持续正确工作（参见 ["可靠性与容错"](#sec_introduction_reliability)）；
* 如何通过高效增加计算资源，让系统在负载增长时保持 **scalability**（参见 ["可伸缩性"](#sec_introduction_scalability)）；以及
* 如何让系统在长期演进中保持 **maintainability**（参见 ["可维护性"](#sec_introduction_maintainability)）。
...
<img src="../../static/fig/ddia_0202.png" alt="图 2-2. 扇出：将新帖子传递给发布帖子的用户的每个追随者。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
如果系统在发生某些故障时仍继续向用户提供所需的服务，我们称系统为 *fault-tolerant*。如果系统不能容忍某个部分变得有故障，我们称该部分为 *single point of failure*（SPOF），因为该部分的故障会升级导致整个系统的失效。

例如在社交网络案例中，扇出流程里可能有机器崩溃或不可用，导致物化时间线更新中断。若要让该流程具备容错性，就必须保证有其他机器可接管任务，同时既不漏投帖子，也不重复投递。（这个思想称为 *exactly-once semantics*，我们会在 [“数据库的端到端论证”](/ch13#sec_future_end_to_end) 中详细讨论。）
...
反直觉的是，在这类系统里，故意 *提高* 故障发生率反而有意义，例如无预警随机杀死某个进程。这叫 *fault injection*。许多关键故障本质上是错误处理做得不够好 [^38]。通过主动注入故障，可以持续演练并验证容错机制，提升对“真实故障发生时系统仍能正确处理”的信心。*chaos engineering* 就是围绕这类实验建立起来的方法论 [^39]。
