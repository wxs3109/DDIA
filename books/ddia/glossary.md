...
>
> 翻译约定：本术语表以英文术语为准，中文仅作解释或索引。后续正文翻译如命中本表术语，默认保留英文，不再把正式术语翻成中文；若需要照顾可读性，可采用“英文术语 + 中文解释”的写法。
...
一个上游请求触发多个下游操作时，请求数量被放大的现象或倍数。在社交网络时间线场景中，一条帖子需要被传播给大量追随者，就是典型的 *fan-out*。参见“[时间线的物化与更新](/ch2#sec_introduction_materializing)”。

### fault

某个 *component* 停止正常工作的情况，例如硬盘损坏、单台机器宕机或依赖服务中断。它描述的是组件层面的异常，不一定已经升级为整个系统不可用。参见“[Reliability 与 Fault Tolerance](/ch2#sec_introduction_reliability)”。

### failure

*system* 无法继续向用户提供所需服务的情况；也就是系统层面未满足既定服务目标。*failure* 常由一个或多个 *fault* 升级而来。参见“[Reliability 与 Fault Tolerance](/ch2#sec_introduction_reliability)”。
...
系统在发生某些 *fault* 时，仍能继续提供所需服务的能力。术语重点在“容忍 fault 而不演变成对用户可见的 failure”。参见“[Reliability 与 Fault Tolerance](/ch2#sec_introduction_reliability)”。
...
从用户发出请求到收到响应所经历的总时间，包含 *service time*、网络传输时间以及各处的 *queueing delay*。参见“[描述性能](/ch2#sec_introduction_percentiles)”和“[Latency 与 Response Time](/ch2#id23)”。

### queueing delay

请求到达后因为资源忙碌而等待的时间，例如等待 CPU、线程、磁盘或网络缓冲区空出来。高负载下，*queueing delay* 往往是 *response time* 恶化的主要原因。参见“[描述性能](/ch2#sec_introduction_percentiles)”和“[Latency 与 Response Time](/ch2#id23)”。
...
系统变慢后，客户端因超时而集中重试，导致负载进一步上升、服务进一步变慢的恶性循环。参见“[系统过载后无法恢复时会发生什么](/ch2#sidebar_metastable)”。
...
延迟分布尾部的高百分位表现，通常关注 *p95*、*p99*、*p999* 等指标。它比平均值更能反映“最慢那部分请求”的用户体验与系统风险。参见“[描述性能](/ch2#sec_introduction_percentiles)”。

### throughput

系统在单位时间内可处理的请求数或数据量。对给定硬件资源而言，存在一个 *maximum throughput*；当负载逼近该上限时，*queueing delay* 往往会快速上升。参见“[描述性能](/ch2#sec_introduction_percentiles)”。
