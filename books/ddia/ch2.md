...
<img src="../../static/map/ch01.png" alt="章节导图" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
构建一个应用时，你通常会从一张需求清单开始。清单最上面的，往往是应用必须提供的功能：需要哪些页面和按钮，每个操作应该完成什么行为，才能实现软件的目标。这些就是 ***functional requirements***。

此外，你通常还会有一些 ***nonfunctional requirements***：例如，应用应当足够快、足够可靠、足够安全、符合法规，而且易于维护。这些需求可能并没有明确写下来，因为它们看起来像是“常识”，但它们与功能需求同样重要。一个慢得无法忍受、或频繁出错的应用，几乎等于不存在。

许多非功能性需求（比如安全）超出了本书范围。但本章会讨论其中几项核心要求，并帮助你用更清晰的方式描述自己的系统：

* 如何定义并衡量系统的 **performance**（参见 ["描述性能"](#sec_introduction_percentiles)）；
* 服务 **reliability** 到底意味着什么：也就是在出错时仍能持续正确工作（参见 ["Reliability 与 Fault Tolerance"](#sec_introduction_reliability)）；
* 如何通过高效增加计算资源，让系统在负载增长时保持 **scalability**（参见 ["Scalability"](#sec_introduction_scalability)）；以及
* 如何让系统在长期演进中保持 **maintainability**（参见 ["Maintainability"](#sec_introduction_maintainability)）。
...
<a id="fig_twitter_relational"></a>

图 2-1. 社交网络的简单关系模式，用户可以相互关注。

<img src="../../static/fig/ddia_0201.png" alt="图 2-1. 社交网络的简单关系模式，用户可以相互关注。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />

假设该社交网络最重要的读操作是 *home timeline*：展示你所追随的人最近发布的帖子（为简化起见，我们忽略广告、未追随用户的推荐帖，以及其他扩展功能）。获取某个用户首页时间线的 SQL 可能如下：
...
帖子具有时效性。我们假设：某人发帖后，追随者应在 5 秒内看到。一个做法是客户端每 5 秒重复执行一次上述查询（即 *polling*）。如果同时在线登录用户有 1000 万，就意味着每秒要执行 200 万次查询。即使把轮询间隔调大，这个量也很可观。
...
这种方法的缺点是：每次发帖时都要做更多工作，因为首页时间线属于需要持续更新的派生数据。这个过程见 [图 2-2](#fig_twitter_timelines)。当一个初始请求触发多个下游请求时，我们用 *fan-out* 描述请求数量被放大的倍数。

<a id="fig_twitter_timelines"></a>

图 2-2. Fan-out：将新帖子传递给发布帖子的用户的每个追随者。

<img src="../../static/fig/ddia_0202.png" alt="图 2-2. Fan-out：将新帖子传递给发布帖子的用户的每个追随者。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />

按每秒 5,700 条帖子计算，若平均每条帖到达 200 名追随者（*fan-out factor* 为 200），则每秒需要略高于 100 万次首页时间线写入。这已经很多，但相比原先每秒 4 亿次“按发送者查帖”，仍是显著优化。

如果遇到特殊事件导致发帖速率激增，我们不必立刻完成时间线投递。可以先入队，接受“帖子出现在追随者时间线中”会暂时变慢。即便在这种峰值期，时间线加载仍然很快，因为读取仍来自缓存。

这种预先计算并持续更新查询结果的过程称为 *materialization*。时间线缓存就是一种 *materialized view*（这个概念见 [“维护物化视图”](/ch12#sec_stream_mat_view)）。物化视图能加速读取，但代价是写入侧工作量增加。对大多数用户而言，这个写入成本仍可接受，但社交网络还要处理一些极端情况：
...
> [!NOTE] Wenbo 注
> 现实中的社交网络通常不会纯粹只选 `fan-in` 或只选 `fan-out`，而是按用户类型和流量形态做取舍。`X/Twitter` 早期公开分享过一种很典型的 `hybrid` 思路：普通账户发帖时，系统主要走 `fan-out on write`，把帖子预先写进追随者的 home timeline，这样读路径很快；但像名人账号这类“单次发帖会触发几百万追随者更新”的场景，写扩散成本太高，就改成把帖子单独保留，等用户真正打开时间线时再和已有缓存结果合并，这一段更接近 `fan-in on read` [^1] [^2] [^6]。取舍逻辑很现实：如果你的读请求极多、用户希望一刷就出来，而且大多数账户的 follower 数量还算可控，那么 `fan-out` 值得，因为它是“用写时成本换读时延迟”；如果某些作者是超级节点，单条内容的扩散成本远超常态，那么这些作者就不适合继续走纯 `fan-out`，否则系统会被少数热点写爆。再比如 `Bluesky` 公开讨论过 *lossy timelines*：当时间线写入压力过大时，允许丢掉一部分本来就很难被用户真正读到的更新 [^5]。这说明现实系统里的 `hybrid` 往往不只是“普通用户走 fan-out、名人走 fan-in”，还会进一步叠加限流、排队、降采样或延迟投递，核心目标不是追求某种教科书上最纯的架构，而是把读延迟、写放大、热点账户、基础设施成本几件事同时压在可接受范围内。
...
throughput
: 系统每秒可处理的请求数或数据量。对于给定硬件资源，系统存在一个可处理的 *maximum throughput*。单位是“每秒某种工作量”。

在社交网络案例中，“每秒帖子数”和“每秒时间线写入数”属于吞吐量指标；“加载首页时间线所需时间”或“帖子送达追随者所需时间”属于响应时间指标。

吞吐量和响应时间之间通常相关。在线服务的典型关系如 [图 2-3](#fig_throughput)：低吞吐量时响应时间较低，负载升高后响应时间上升。原因是 *queueing*。请求到达高负载系统时，CPU 往往已在处理前一个请求，新请求只能等待；当吞吐量逼近硬件上限，排队延迟会急剧上升。

<a id="fig_throughput"></a>

图 2-3. 随着服务的吞吐量接近其容量，由于排队，响应时间急剧增加。

<img src="../../static/fig/ddia_0203.png" alt="图 2-3. 随着服务的吞吐量接近其容量，由于排队，响应时间急剧增加。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />

> 注：响应时间 = 服务时间 + 排队时间。低负载时几乎不用排队，因此响应时间接近空载时的服务时间；当吞吐量逼近硬件上限时，请求开始堆积，排队时间会急剧上升。
...
如果系统已接近过载、吞吐量逼近极限，有时会进入恶性循环：效率下降，进而更加过载。例如，请求队列很长时，响应时间可能高到让客户端超时并重发请求，导致请求速率进一步上升，问题持续恶化，形成 *retry storm*。即使负载后来回落，系统也可能仍卡在过载状态，直到重启或重置。这种现象叫 *metastable failure*，可能引发严重生产故障 [^7] [^8]。

为了避免重试把服务拖垮，可以在客户端拉大并随机化重试间隔（*exponential backoff* [^9] [^10]），并临时停止向近期报错或超时的服务发请求（例如 *circuit breaker* [^11] [^12] 或 *token bucket* [^13]）。服务端也可在接近过载时主动拒绝请求（*load shedding* [^14]），并通过响应要求客户端降速（*backpressure* [^1] [^15]）。此外，排队与负载均衡算法的选择也会影响结果 [^16]。

--------

从性能指标角度看，用户通常最关心响应时间；而吞吐量决定了所需计算资源（例如服务器数量），从而决定承载特定工作负载的成本。如果吞吐量增长可能超过当前硬件上限，就必须扩容；若系统可通过增加计算资源显著提升最大吞吐量，就称其 *scalable*。

本节主要讨论 *response time*；*throughput* 与 ["Scalability"](#sec_introduction_scalability) 会在后文再展开。

### Latency 与 Response Time {#id23}

*latency* 与 *response time* 有时会混用，但本书对它们有明确区分（见 [图 2-4](#fig_response_time)）：

* *response time* 是客户端看到的总时间，包含链路上各处产生的全部延迟。
* *service time* 是服务主动处理该请求的时间。
* *queueing delay* 可发生在流程中的多个位置。例如请求到达后，可能要等 CPU 空出来才能处理；同机其他任务若占满出站网卡，响应包也可能先在缓冲区等待发送。
* *latency* 是对“请求未被主动处理这段时间”的统称，也就是请求处于 *latent* 状态的时间。尤其是 *network latency*（或网络时延）指请求与响应在网络中传播所花的时间。

<a id="fig_response_time"></a>

图 2-4. 响应时间、服务时间、网络延迟和排队延迟。

<img src="../../static/fig/ddia_0204.png" alt="图 2-4. 响应时间、服务时间、网络延迟和排队延迟。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
排队延迟常常是响应时间波动的主要来源。服务器并行处理能力有限（例如受 CPU 核数约束），少量慢请求就可能堵住后续请求，这就是 *head-of-line blocking*。即便后续请求本身服务时间很短，客户端仍会因为等待前序请求而看到较慢的总体响应。排队延迟不属于服务时间，因此必须在客户端侧测量响应时间。

### 平均值、中位数与百分位点 {#id24}

由于响应时间会随请求变化，我们应将其看作一个可测量的 *distribution*，而非单一数字。在 [图 2-5](#fig_lognormal) 中，每个灰色柱表示一次请求，柱高是该请求耗时。大多数请求较快，但会有少量更慢的 *outliers*。网络时延波动也常称为 *jitter*。

<a id="fig_lognormal"></a>

图 2-5. 说明平均值和百分位点：100 个服务请求的响应时间样本。

<img src="../../static/fig/ddia_0205.png" alt="图 2-5. 说明平均值和百分位点：100 个服务请求的响应时间样本。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />

报告服务 *mean* 响应时间很常见（严格说是 *arithmetic mean*：总响应时间除以请求数）。平均值对估算吞吐量上限有帮助 [^18]。但若你想知道“典型”响应时间，平均值并不理想，因为它不能反映到底有多少用户经历了这种延迟。

通常，*percentiles* 更有意义。把响应时间从快到慢排序，*median* 位于中间。例如中位响应时间为 200 毫秒，表示一半请求在 200 毫秒内返回，另一半更慢。因此中位数适合衡量用户“通常要等多久”。中位数也称 *50th percentile*，常记为 *p50*。
...
响应时间的高百分位点（也叫 *tail latency*）非常重要，因为它直接影响用户体验。例如亚马逊内部服务常以第 99.9 百分位设定响应要求，尽管它只影响 1/1000 的请求。原因是最慢请求往往来自“账户数据最多”的客户，他们通常也是最有价值客户 [^19]。让这批用户也能获得快速响应，对业务很关键。

另一方面，继续优化到第 99.99 百分位（最慢的万分之一请求）通常成本过高、收益有限。越到高百分位，越容易受不可控随机因素影响，也更符合边际收益递减规律。

> 注（近年业界实践）：面向 Web 的用户体验指标，常先保证“大多数真实用户”而非极端尾部。例如 Google Core Web Vitals 采用真实用户数据的 p75 作为达标阈值：LCP ≤ 2.5 秒、INP ≤ 200 毫秒、CLS ≤ 0.1 [^99] [^100] [^101]。而在后端服务的 SRE 实践里，延迟目标仍常写成 p99 或 p99.9，并明确建议避免把目标设成 100% 这类代价极高的绝对值 [^102]。
...
对于“一个终端请求会触发多次后端调用”的服务，高百分位点尤其关键。即使并行调用，终端请求仍要等待最慢的那个返回。正如 [图 2-6](#fig_tail_amplification) 所示，只要一个调用慢，就能拖慢整个终端请求。即便慢调用比例很小，只要后端调用次数变多，撞上慢调用的概率就会上升，于是更大比例的终端请求会变慢（称为 *tail-latency amplification* [^26]）。

<a id="fig_tail_amplification"></a>

图 2-6. 当需要几个后端调用来服务请求时，只需要一个慢的后端请求就可以减慢整个最终用户请求。

<img src="../../static/fig/ddia_0206.png" alt="图 2-6. 当需要几个后端调用来服务请求时，只需要一个慢的后端请求就可以减慢整个最终用户请求。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />

百分位点也常用于定义 *service level objective*（SLO）和 *service level agreement*（SLA）[^27]。例如，一个 SLO 可能要求：中位响应时间低于 200 毫秒、p99 低于 1 秒，并且至少 99.9% 的有效请求返回非错误响应。SLA 则是“未达成 SLO 时如何处理”的合同条款（例如客户可获赔偿）。这是基本思路；但在实践中，为 SLO/SLA 设计合理可用性指标并不容易 [^28] [^29]。
...
## Reliability 与 Fault Tolerance {#sec_introduction_reliability}
...
如果把这些合起来称为“正确工作”，那么 *reliability* 可以粗略理解为：即使出现问题，系统仍能持续正确工作。为了更精确地描述“出问题”，我们区分 *fault* 与 *failure* [^35] [^36] [^37]：

fault
: 指系统某个 *component* 停止正常工作：例如单个硬盘损坏、单台机器宕机，或系统依赖的外部服务中断。

failure
: 指 *system* 无法继续向用户提供所需服务；换言之，系统未满足服务级别目标（SLO）。

*fault* 与 *failure* 的区别容易混淆，因为它们本质上是同一件事在不同层级上的表述。比如一个硬盘坏了，对“硬盘这个系统”来说是 *failure*；但对“由许多硬盘组成的更大系统”来说，它只是一个 *fault*。更大系统若在其他硬盘上有副本，就可能容忍该 *fault*。

### Fault Tolerance {#id27}

如果系统在发生某些 *fault* 时仍继续向用户提供所需的服务，我们称系统具备 *fault tolerance*，或称其为 *fault-tolerant*。如果系统不能容忍某个部分变得有故障，我们称该部分为 *single point of failure*（SPOF），因为该部分的 *fault* 会升级导致整个系统的 *failure*。

例如在社交网络案例中，*fan-out* 流程里可能有机器崩溃或不可用，导致物化时间线更新中断。若要让该流程具备 *fault tolerance*，就必须保证有其他机器可接管任务，同时既不漏投帖子，也不重复投递。（这个思想称为 *exactly-once semantics*，我们会在 [“数据库的端到端论证”](/ch13#sec_future_end_to_end) 中详细讨论。）

*Fault tolerance* 总是“有边界”的：它只针对某些类型、某个数量以内的 *fault*。例如系统可能最多容忍 2 块硬盘同时故障，或 3 个节点里坏 1 个。若全部节点都崩溃，就无计可施，因此“容忍任意数量 fault”并无意义。要是地球和上面的服务器都被黑洞吞噬，那就只能去太空托管了，预算审批祝你好运。

反直觉的是，在这类系统里，故意 *提高* *fault* 发生率反而有意义，例如无预警随机杀死某个进程。这叫 *fault injection*。许多关键 *failure* 本质上是错误处理做得不够好 [^38]。通过主动注入 *fault*，可以持续演练并验证 *fault tolerance* 机制，提升对“真实 fault 发生时系统仍能正确处理”的信心。*chaos engineering* 就是围绕这类实验建立起来的方法论 [^39]。

尽管我们通常更倾向于“容忍故障”，而非“阻止故障”，但也有“预防优于补救”的场景（例如根本无法补救）。安全问题就是如此：若攻击者已攻破系统并获取敏感数据，事件本身无法撤销。不过，本书主要讨论的是可恢复的故障类型。

### Hardware Faults 与 Software Faults {#sec_introduction_hardware_faults}
...
#### 通过冗余容忍 Hardware Faults {#tolerating-hardware-faults-through-redundancy}
...
硬件冗余确实能提升单机可用时间；但正如 ["分布式与单节点系统"](/ch1#sec_introduction_distributed) 所述，分布式系统还具备额外优势，例如可容忍整个数据中心中断。因此云系统通常不再过分追求“单机极致可靠”，而是通过软件层容忍节点故障来实现高可用。云厂商使用 *availability zone* 标识资源是否物理共址；同一可用区内资源比跨地域资源更容易同时失效。
...
能够容忍整个机器丢失的系统也具有运营优势：如果你需要重新启动机器（例如，应用操作系统安全补丁），单服务器系统需要计划停机时间，而多节点容错系统可以一次修补一个节点，而不影响用户的服务。这称为 *rolling upgrade*，我们将在 [第 5 章](/ch5) 中进一步讨论它。
...
有多种技术手段可降低人为失误的影响：充分测试（含手写测试与大量随机输入的 *property-based testing*）[^38]、可快速回滚配置变更的机制、新代码渐进发布、清晰细致的监控、用于排查生产问题的可观测性工具（参见 ["分布式系统的问题"](/ch1#sec_introduction_dist_sys_problems)），以及鼓励“正确操作”并抑制“错误操作”的良好界面设计。
...
越来越多组织在实践 *blameless postmortem*：事故发生后，鼓励参与者在不担心惩罚的前提下完整复盘细节，让组织其他人也能学习如何避免类似问题 [^73]。这个过程常会揭示出：业务优先级需要调整、某些长期被忽视的领域需要补投入、相关激励机制需要改，或其他应由管理层关注的系统性问题。
...
*scalability* 用来描述系统应对负载增长的能力。讨论这个话题时，常有人说：“你又不是 Google/Amazon，别担心规模，直接上关系数据库。”这句话是否成立，取决于你在做什么类型的应用。
...
> Wenbo 注：这两个问题看起来相似，但关注点不同。第一个是在问“现有系统扛到什么程度会变慢”：例如数据库服务器仍是 8 核、32 GB 内存不变，把每秒请求数从 1 万提高到 2 万后，p99 会不会从 80 ms 升到 400 ms。第二个是在问“为了维持原有体验，要加多少资源”：例如希望 p99 继续维持在 100 ms 内，那么请求量翻倍时，是否需要把 4 台应用服务器扩到 8 台，或把单节点从 8 核升级到 16 核。

通常目标是：在尽量降低运行成本的同时，让性能维持在 SLA 要求内（参见 ["响应时间指标的应用"](#sec_introduction_slo_sla)）。所需计算资源越多，成本越高。不同硬件的性价比不同，而且会随着新硬件出现而变化。

如果资源翻倍后能承载两倍负载且性能不变，这称为 *linear scalability*，通常是理想状态。偶尔，借助规模效应或峰值负载更均匀分布，甚至可用不足两倍资源处理两倍负载 [^79] [^80]。但更常见的是成本增长快于线性，低效原因也很多。比如数据量增大后，即使请求大小相同，处理一次写请求也可能比数据量小时更耗资源。

### 共享内存、共享磁盘与无共享架构 {#sec_introduction_shared_nothing}

增加服务硬件资源的最简单方式，是迁移到更强的机器。虽然单核 CPU 不再明显提速，但你仍可购买（或租用）拥有更多 CPU 核心、更多 RAM、更多磁盘的实例。这叫 *scaling up*。

在单机上，你可以通过多进程/多线程获得并行性。同一进程内线程共享同一块 RAM，因此这也叫 *shared-memory architecture*。问题是它的成本常常“超线性增长”：硬件资源翻倍的高端机器，价格往往远超两倍；且受限于瓶颈，性能提升通常又达不到两倍。

另一种方案是 *shared-disk architecture*：多台机器各有独立 CPU 和 RAM，但共享同一组磁盘阵列，通过高速网络连接（NAS 或 SAN）。该架构传统上用于本地数据仓库场景，但争用与锁开销限制了其可伸缩性 [^81]。

相比之下，*shared-nothing architecture* [^82]（即 *scaling out*）已广泛流行。这种方案使用多节点分布式系统，每个节点拥有自己的 CPU、RAM 和磁盘；节点间协作通过常规网络在软件层完成。

> Wenbo 注：这里最容易混淆的是 `scale up` 和 `scale out`。`scale up` 是“把单机做大”：例如原来用 1 台 8 核、32 GB 内存的数据库机，现在换成 1 台 32 核、128 GB 内存的更强机器，应用拓扑基本不变，主要变化是单机规格更高。`scale out` 是“把机器变多”：例如不再指望 1 台大机器扛住全部流量，而是改成 8 台普通节点分担请求或数据，每台只处理一部分。前者通常实现更直接，但会遇到高端机器价格更贵、单机上限更明显的问题；后者理论上更容易继续扩容，但代价是要处理分片、复制、再平衡、跨节点协调等分布式复杂性。
...
能够大规模运行的系统架构，通常高度依赖具体应用，不存在通用“一招鲜”的可伸缩架构（俗称 *silver bullet*）。例如：面向“每秒 10 万次请求、每次 1 kB”的系统，与面向“每分钟 3 次请求、每次 2 GB”的系统，形态会完全不同，尽管二者数据吞吐量都约为 100 MB/s。

> Wenbo 注：这类差异正是系统设计面试很爱考的点。面试官可能不会直接问“你怎么看这句话”，而会换成更具体的问题，比如：“设计一个图片/视频上传系统，请求频率不高但单次对象很大，你会怎么处理存储、带宽和异步任务？”或者“设计一个聊天/通知系统，消息很小但 QPS 极高，你会怎么做连接管理、扇出和水平扩展？”表面上两者总吞吐量都可能差不多，但前者更容易先撞上网络带宽、磁盘吞吐和大对象传输问题，后者更容易先撞上连接数、CPU 调度、缓存热点与 fan-out 压力。面试里真正要先问清楚的，通常不是‘总流量有多大’，而是‘请求有多频繁、单次请求有多重、峰值长什么样’。
...
然而维护并不容易。一个长期运行成功的系统，可能仍依赖今天少有人熟悉的旧技术（如大型机和 COBOL）；随着人员流动，系统为何如此设计的组织记忆也可能丢失；维护者往往还要修复前人留下的问题。更重要的是，计算机系统通常与其支撑的组织流程深度耦合，这使得 *legacy* 系统维护既是技术问题，也是人员与组织问题 [^87]。
...
小型项目往往能保持简洁、优雅、富有表达力；但项目变大后，代码常会迅速变复杂且难理解。这种复杂性会拖慢所有参与者效率，进一步抬高维护成本。陷入这种状态的软件项目常被称为 *big ball of mud* [^91]。
...
一种常见分析方法是把复杂性分成两类：**essential complexity** 与 **accidental complexity** [^93]。前者源于业务问题本身，后者源于工具与实现限制。但这种划分也并不完美，因为随着工具演进，“本质”和“偶然”的边界会移动 [^94]。

> [!NOTE] Wenbo 注
> 可以用一个很直观的例子来区分。假设你要做“多人协作文档编辑”：多人同时修改同一份文档、冲突怎么解决、谁先谁后如何定义，这些问题即使用最好的语言和框架也仍然存在，属于 `essential complexity`，因为它来自业务本身。相对地，如果你只是想把一个用户表查出来，却还要手写大量样板 SQL、对象映射、序列化和胶水代码，甚至还得绕开某个框架的奇怪限制，这部分更像 `accidental complexity`，因为它主要是工具链和实现方式带来的。换句话说，前者是“这个问题天生就难”，后者是“本来没那么难，但被工具和实现放大了”。

管理复杂度最重要的工具之一是 **abstraction**。好的抽象能在清晰外观后隐藏大量实现细节，也能被多种场景复用。这种复用不仅比反复重写更高效，也能提升质量，因为抽象组件一旦改进，所有依赖它的应用都会受益。
...
应用代码层面的抽象，常借助 *design patterns* [^95]、*domain-driven design*（DDD）[^96] 等方法来构建。本书重点不在这类应用专用抽象，而在你可以拿来构建应用的通用抽象，例如数据库事务、索引、事件日志等。若你想采用 DDD 等方法，也可以建立在本书介绍的基础能力之上。
...
在组织层面，*agile* 方法为适应变化提供了框架；敏捷社区也发展出多种适用于高变化环境的技术与流程，如测试驱动开发（TDD）和重构。本书关注的是：如何在“由多个不同应用/服务组成的系统层级”提升这种敏捷能力。

数据系统对变化的适应难易度，与其简单性和抽象质量高度相关：松耦合、简单系统通常比紧耦合、复杂系统更容易修改。由于这一点极其重要，我们把“数据系统层面的敏捷性”单独称为 *evolvability* [^97]。
...
为实现可靠性，可以使用容错机制，使系统在部分组件（如磁盘、机器或外部服务）故障时仍能持续提供服务。我们区分了硬件故障与软件故障，并指出软件故障常更难处理，因为它们往往高度相关。可靠性的另一面是“对人为失误的韧性”，其中 *blameless postmortem* 是重要学习机制。
...
[^99]: Philip Walton and Barry Pollard. [Largest Contentful Paint (LCP)](https://web.dev/articles/lcp). *web.dev*, originally published August 2019, last updated September 2025. 
[^100]: Jeremy Wagner and Barry Pollard. [Interaction to Next Paint (INP)](https://web.dev/articles/inp). *web.dev*, originally published May 2022, last updated September 2025. 
[^101]: Milica Mihajlija and Philip Walton. [Cumulative Layout Shift (CLS)](https://web.dev/articles/cls). *web.dev*, April 2023. 
[^102]: Chris Jones, John Wilkes, and Niall Murphy. [Service Level Objectives](https://sre.google/sre-book/service-level-objectives/). In *Site Reliability Engineering*. Google and O'Reilly Media, 2017. 
