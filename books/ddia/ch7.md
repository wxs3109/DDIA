...
一个节点可能存储多个 shard。例如，如果使用单leader复制模型，sharding 与复制的组合可能如 [图 7-1](#fig_sharding_replicas) 所示。每个 shard 的leader被分配到一个节点，follower被分配到其他节点。每个节点可能是某些 shard 的leader，同时又是其他 shard 的follower，但每个 shard 仍然只有一个leader。

<a id="fig_sharding_replicas"></a>

图 7-1. 复制与 sharding 结合使用：每个节点对某些 shard 充当leader，对另一些 shard 充当follower。

<img src="../../static/fig/ddia_0701.png" alt="图 7-1. 复制与 sharding 结合使用：每个节点对某些 shard 充当leader，对另一些 shard 充当follower。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
Sharding 的另一个问题是写入可能需要更新多个不同 shard 中的相关记录。虽然 *single-node transaction* 相当常见（见 [第 8 章](/ch8#ch_transactions)），但确保跨多个 shard 的一致性需要 *distributed transaction*。正如我们将在 [第 8 章](/ch8#ch_transactions) 中看到的，*distributed transaction* 在某些数据库中可用，但它们通常比 *single-node transaction* 慢得多，可能成为整个系统的瓶颈，有些系统根本不支持它们。

> [!CAUTION] Wenbo 注
> 这里的关键区别是：如果一次写入只落在一个 shard 内，数据库通常只需要在单个节点内完成提交或回滚，这就是 *single-node transaction*。但如果一个业务动作要同时修改多个 shard，系统就必须协调多个节点一起提交或一起回滚，这就是 *distributed transaction*。这种协调往往涉及额外的网络往返、锁等待或类似 two-phase commit 的协议，因此延迟更高，也更容易成为瓶颈。
...
> [!CAUTION] Wenbo 注
> 对 sharding 来说，哈希函数最重要的性质之一不是“安全”，而是 *stable*：同一个 key 无论在哪个进程、哪台机器、哪次重启之后，都应该得到同一个 hash 值。否则，不同节点会对“这个 key 属于哪个 shard”得出不同结论。
>
> 编程语言内置的 hash 往往是为进程内的 hash table 优化的，而不是为分布式路由设计的。运行时有时会引入随机 seed，或者允许不同进程采用不同的内部实现；这样做有助于性能或安全，但代价是 hash 值不再能跨进程稳定复现。结果就是：应用今天把某个 key 写到 shard 3，明天另一个进程可能会认为同一个 key 应该去 shard 7，进而造成错误路由，甚至让数据看起来像“丢了”。
...
> [!CAUTION] Wenbo 注
> 读这张图时，可以把下面的长条看成整个 hash space（这里简化为 0 到 1024）。上半部分表示加入 Node 3 之前，hash space 已经被若干边界切成多个连续 range，每个 range 归属于 n0、n1 或 n2；上面的三个大框则是在按节点汇总：例如 Node 1 拥有 0-88、511-672 和 702-930 这三个不相邻的 range。
>
> 加入 Node 3 后，并不是把所有 key 重新 hash 一遍，也不是把所有数据平均重洗。系统只新增几个边界（图中是 60、276、551），把原有的部分 range 切开，然后把切出来的一些子 range 分给 n3。这里的 range 指的是 hash value 的范围：例如 551-672 这段交给 Node 3，意思是原来在 Node 1 上、所有满足 551 <= hash(key) < 672 的记录都要迁移到 Node 3；hash 值落在 511-551 的记录仍留在 Node 1。具体来说：原来 Node 1 负责 0-88，现在 60-88 这段被切出来交给 Node 3，Node 1 只保留 0-60；原来 Node 2 负责 88-309，现在 276-309 被切出来交给 Node 3，Node 2 保留 88-276；原来 Node 1 还负责 511-672，现在 551-672 被切出来交给 Node 3，Node 1 保留 511-551。于是 Node 3 新拿到的就是 60-88、276-309、551-672 这三段数据。
>
> 没有被切到的 range 不需要移动：例如 Node 0 的那些 range，以及 Node 1 的 702-930、Node 2 的 672-702 和 930-1024 仍然留在原来的节点上。这就是 consistent hashing 想要的效果：节点数变化时，只移动必要的一小部分 key，而不是让大多数 key 改变归属。
...
> [!CAUTION] Wenbo 注
> 这里说的“附加随机数”，不是改变业务语义，而是把一个逻辑上的 hot key 拆成多个物理 key。比如原来的 key 是 `celebrity:42:comments`，所有评论写入都会打到同一个 partition key 上；现在写入时随机选一个 00-99 的后缀，变成 `celebrity:42:comments:00`、`celebrity:42:comments:37`、`celebrity:42:comments:99` 等 100 个 key。由于这些 key 的 hash 值通常会落到不同 shard 上，写入压力就从“一个 shard 扛全部”变成“大约 100 个 key 分摊”。
>
> 代价是读取变贵：如果你想读完整的评论列表，就不能只读 `celebrity:42:comments` 这一个 key，而要把 00-99 这 100 个 bucket 都查出来再合并。因此这种方法适合“写入特别热、读取可以接受 fan-out”的场景；如果读取也很热，就要再配合缓存、预聚合或专门的 hot key 管理策略。
