...
> [!CAUTION] Wenbo 注
>
> C# 的 Entity Framework/EF Core 默认也会有这个问题。典型写法是先 `Find`/`Single` 把 entity 读进 change tracker，在 C# 里执行 `counter.Value++` 或 `account.Balance -= amount`，然后 `SaveChanges()`；EF 通常生成的是 `UPDATE ... SET Value = @p`，也就是把应用层算出的新值写回去，而不是数据库侧的 `SET Value = Value + 1`。如果两个 request 都基于同一个旧值计算，后提交的那个会覆盖前一个，除非你额外配置 optimistic concurrency token（例如 SQL Server 的 `rowversion`/`[Timestamp]`，冲突时抛 `DbUpdateConcurrencyException` 并 retry）、使用数据库侧 atomic update（EF Core 7+ 的 `ExecuteUpdate` 可以写成 `SetProperty(e => e.Value, e => e.Value + 1)`，或用 raw SQL），或者提高 isolation/显式加锁。`SaveChanges` 自带 transaction 只能保证这次写入原子提交，不能自动防止 lost update。
...
atomic operation 和 lock 是通过强制 read-modify-write cycle 按顺序发生来防止 lost update 的方法。另一种选择是允许它们 parallel 执行，如果 transaction manager 检测到 lost update，则 abort 其中一个 transaction 并强制它 retry read-modify-write cycle。
...
> [!CAUTION] Wenbo 注
>
> 这里可以把 lost update 想成一个 wiki 编辑场景：页面一开始是 `version = 1`，A 和 B 同时打开页面，所以两个人手里的“原始版本”都是 `1`。B 先保存，数据库把内容改掉并把 version 增加到 `2`。如果 A 后保存时只是执行 `UPDATE wiki_pages SET content = 'A 的内容' WHERE id = 1234`，那 A 就会把 B 的修改覆盖掉，这就是 lost update。
>
> 正确的检查逻辑正是“对照目前的版本和 A 手里最初的版本是否一样”：A 保存时应该带上自己读到的旧 version，例如 `UPDATE wiki_pages SET content = 'A 的内容', version = version + 1 WHERE id = 1234 AND version = 1`。如果 B 已经提交过，数据库里的 version 已经是 `2`，这个 update 就会影响 0 行；应用看到 0 行，就知道 A 基于旧版本编辑，需要 retry、提示用户 merge，或者让用户确认覆盖。这个思路就是下面的 conditional write / compare-and-set，也常叫 optimistic locking。
>
> 需要注意的是，真实 wiki/表单编辑通常不会从用户打开页面到点击保存一直持有一个数据库 transaction，因为用户可能编辑很久。更常见的做法是：打开页面时读出内容和 version，把 version 随表单一起带回；保存时用 `WHERE id = ... AND version = old_version` 做条件更新。数据库自动检测 lost update 是另一层保护：在某些 isolation level 下，数据库会在 transaction 内发现你基于旧 snapshot 更新了已被别人改过的行并 abort；但不是所有数据库和 isolation level 都保证这一点，所以应用层 version check 仍然很常见。
...
> [!CAUTION] Wenbo 注
>
> 图 8-8 的关键不是“两个人同时改了同一行”，而是“两个人基于同一个全局判断，各自改了不同的行”。Aaliyah 的 transaction 读到：`Aaliyah = on_call, Bryce = on_call`，所以它判断“还有 2 个医生值班，我可以把自己改成 off_call”。Bryce 的 transaction 在自己的 snapshot 里也读到完全相同的状态，所以它也判断“还有 2 个医生值班，我也可以把自己改成 off_call”。
>
> snapshot isolation 的问题在于：两个 transaction 的 read 都停留在各自开始时的 snapshot 上。即使 Bryce 先 commit，Aaliyah 在自己的 transaction 里也不会重新看到“Bryce 已经下班，所以只剩我一个医生”的新事实。并且它们写的是不同 row：Aaliyah 只更新 Aaliyah 那行，Bryce 只更新 Bryce 那行，所以数据库也没有发现“同一行被两个事务同时覆盖”的 lost update conflict。两个局部 update 都合法，但合起来破坏了跨 row 的 invariant：`COUNT(on_call doctors) >= 1`。
>
> 如果这两个操作 serially 执行，结果就不会错：假设 Bryce 先完成，Aaliyah 后开始，那么 Aaliyah 的检查会看到只剩 1 个医生值班，因此应该拒绝下班。write skew 的危险就在这里：每个 transaction 单独看都符合业务规则，但并发执行时，因为规则依赖的是一组 row 的整体状态，而不是某一行的当前值，snapshot isolation 不一定能替你阻止它。要避免这种情况，通常需要 serializable isolation、显式锁住决策依赖的 rows（例如 `SELECT ... FOR UPDATE`），或把 invariant 交给数据库能强制执行的 constraint/trigger。
