...
<img src="../../static/map/ch03.png" alt="章节导图" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_csv_hash_index"></a>

图 4-1. 以类似 CSV 格式存储键值对日志，使用内存哈希映射建立索引。

<img src="../../static/fig/ddia_0401.png" alt="图 4-1. 以类似 CSV 格式存储键值对日志，使用内存哈希映射建立索引。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_sstable_index"></a>

图 4-2. 带有稀疏索引的 SSTable，允许查询跳转到正确的块。

<img src="../../static/fig/ddia_0402.png" alt="图 4-2. 带有稀疏索引的 SSTable，允许查询跳转到正确的块。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_sstable_merging"></a>

图 4-3. 合并多个 SSTable 段，仅保留每个键的最新值。

<img src="../../static/fig/ddia_0403.png" alt="图 4-3. 合并多个 SSTable 段，仅保留每个键的最新值。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_bloom"></a>

图 4-4. 布隆过滤器提供了一种快速的概率检查，用于判断特定键是否存在于特定 SSTable 中。

<img src="../../static/fig/ddia_0404.png" alt="图 4-4. 布隆过滤器提供了一种快速的概率检查，用于判断特定键是否存在于特定 SSTable 中。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_b_tree"></a>

图 4-5. 使用 B 树索引查找键 251。从根页开始，我们首先跟随引用到键 200–300 的页，然后是键 250–270 的页。

<img src="../../static/fig/ddia_0405.png" alt="图 4-5. 使用 B 树索引查找键 251。从根页开始，我们首先跟随引用到键 200–300 的页，然后是键 250–270 的页。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_storage_b_tree_split"></a>

图 4-6. 通过在边界键 337 上分割页来增长 B 树。父页被更新以引用两个子页。

<img src="../../static/fig/ddia_0406.png" alt="图 4-6. 通过在边界键 337 上分割页来增长 B 树。父页被更新以引用两个子页。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
如果你获取在某个工作负载中写入磁盘的总字节数，然后除以如果你只是写入没有索引的仅追加日志需要写入的字节数，你就得到了 *write amplification*。（有时 write amplification 是根据 I/O 操作而不是字节来定义的。）在写入密集型应用程序中，瓶颈可能是数据库可以写入磁盘的速率。在这种情况下，write amplification 越高，它在可用磁盘带宽内可以处理的每秒写入次数就越少。

> [!NOTE] Wenbo 注
> `write amplification` 指的是：应用层以为自己只写了一点数据，但 storage engine 为了维护索引、日志、页、压实等内部结构，实际写到磁盘上的数据量被“放大”了。
>
> 一个粗略例子：假设应用执行一次 update，只改了 `1 KB` 的用户资料。如果没有索引、没有复杂存储结构，理想情况下只追加写入 `1 KB` 日志就够了。但真实数据库可能还要写：
>
> ```text
> 应用实际修改的数据:       1 KB
> 写入 WAL / commit log:    1 KB
> 更新 B-tree page:         16 KB
> 更新 secondary index page:16 KB
> --------------------------------
> 磁盘实际写入总量:         34 KB
> ```
>
> 那么这次写入的 write amplification 大约就是 `34 KB / 1 KB = 34x`。也就是说，应用逻辑上写了 `1 KB`，底层磁盘却忙了 `34 KB` 的写入工作。
>
> 对 LSM tree 来说，write amplification 通常来自另一种地方：同一条记录先写入 commit log，再写入 memtable flush 出来的 SSTable，之后还可能在 compaction 中被反复重写。对 B-tree 来说，它常来自 WAL、整页写入、page split、secondary index 更新等。这个指标重要，是因为磁盘/SSD 的写入带宽是有限的；write amplification 越高，同样的硬件每秒能承受的应用写入就越少，SSD 磨损也越快。

Write amplification 是 LSM 树和 B 树中的问题。哪个更好取决于各种因素，例如键和值的长度，以及你覆盖现有键与插入新键的频率。对于典型的工作负载，LSM 树往往具有较低的 write amplification，因为它们不必写入整个页，并且可以压缩 SSTable 的块 [^40]。这是使 LSM 存储引擎非常适合写入密集型工作负载的另一个因素。

除了影响吞吐量，write amplification 也与 SSD 的磨损有关：write amplification 较低的存储引擎将更慢地磨损 SSD。

在测量存储引擎的写入吞吐量时，重要的是要运行足够长的实验，以便 write amplification 的影响变得清晰。当写入空的 LSM 树时，还没有进行压实，因此所有磁盘带宽都可用于新写入。随着数据库的增长，新写入需要与压实共享磁盘带宽。
...
> [!NOTE] Wenbo 注
> B-tree fragmentation 是说磁盘文件里出现了很多“空洞”或不再紧凑的 page。比如删除大量 rows 后，某些 B-tree page 里可能只剩少量 key；这些 page 以后可以被数据库复用，但文件本身通常不会立刻变小，因为这些 page 位于文件中间，操作系统不能只把中间一小块空间收回去。
>
> 常见处理方式有几类：
>
> 1. `page reuse`：数据库把空闲 page 记录下来，后续 insert/update 可以重新使用这些 page。这能避免文件继续膨胀，但不一定让文件立刻变小。
> 2. `VACUUM` / background cleanup：像 PostgreSQL 的 autovacuum 会清理 dead tuples，让空间可以被后续写入复用。不过普通 `VACUUM` 通常主要是“标记可复用空间”，不一定把文件归还给操作系统。
> 3. `REINDEX` / rebuild index：重建 B-tree，把仍然有效的 key 重新写成一棵更紧凑的新树。这能减少 index bloat，但成本较高，可能消耗 CPU、I/O，并在某些数据库里需要锁。
> 4. `CLUSTER` / table rewrite / online compaction：把表或索引整体重写，让数据和索引重新排布得更连续、更紧凑。有些数据库支持在线 compaction，有些需要维护窗口。
> 5. `fillfactor`：预留一部分 page 空间，给未来 update/insert 留余地，减少 page split 和后续碎片化。代价是初始占用更多磁盘空间。
>
> 所以 B-tree 的策略通常不是像 LSM tree 那样天然通过 compaction 持续重写 SSTable，而是“尽量复用空 page；必要时重建或重写”。这也是为什么长期大量 delete/update 的 OLTP 表可能会出现 `index bloat`，需要 VACUUM、REINDEX 或类似维护任务。
...
> [!NOTE] Wenbo 注
> `primary index` / 主键索引解决的是“按这行自己的 ID 找到这一行”。例如 `users.id` 唯一标识一个用户，数据库通常会自动为 primary key 建索引：
>
> ```sql
> CREATE TABLE users (
...
> ```
>
> `secondary index` / 二级索引解决的是“按非主键字段找行”。例如一个用户有很多职位，`positions.id` 是每条职位记录自己的主键，但我们经常要按 `user_id` 找到某个用户的所有职位：
>
> ```sql
...
> ```
>
> 这里 `positions_user_id_idx` 就是 secondary index，因为它不是用来唯一标识 `positions` 表里的一行，而是用来按另一个查询条件快速定位一批行。同一个 `user_id` 可能对应多条 `positions`，所以 secondary index 的一个 key 往往会对应多个 row id。没有这个 index，数据库可能要扫描整张 `positions` 表才能找出 `user_id = 251` 的记录。
...
* 如果实际数据（行、文档、顶点）直接存储在索引结构中，则称为 *clustered index*（聚簇索引）。例如，在 MySQL 的 InnoDB 存储引擎中，表的主键始终是 clustered index，在 SQL Server 中，你可以为每个表指定一个 clustered index [^43]。

> [!NOTE] Wenbo 注
> `clustered index` 可以理解为：表的数据本身就存放在这个 index 里面，而且通常按这个 index key 的顺序组织。它不是“旁边额外建一本索引”，而是“表的实际存储结构就是这棵 index tree”。
>
> 在 B-tree 里，普通 index 的 leaf node 可能只存 key 和 row reference；而 clustered index 的 leaf node 直接存完整 row。也就是说，查到 leaf node 时，数据本身已经在那里了。
>
> 例如 SQL Server 可以显式创建 clustered index：
>
> ```sql
> CREATE TABLE users (
>     id bigint NOT NULL,
...
> );
>
> CREATE CLUSTERED INDEX users_id_clustered_idx ON users (id);
> ```
>
> 这表示 `users` 表的实际 rows 会按 `id` 这棵 clustered index 来组织。按 `id` 查询时，查到 B-tree leaf node 就拿到了完整 row：
>
> ```sql
> SELECT * FROM users WHERE id = 251;
> ```
>
> MySQL InnoDB 里更常见的是 primary key 自动成为 clustered index：
>
> ```sql
> CREATE TABLE users (
>     id bigint PRIMARY KEY,
...
> ```
>
> 在这个例子里，`id` 是 primary key，也是 InnoDB 的 clustered index。`users_email_idx` 是 secondary index。按 `email` 查询时，数据库通常先在 secondary index 里找到对应的 primary key，然后再回到 clustered index 里取完整 row：
>
> ```text
> users_email_idx: email -> id
> clustered index: id -> full row
> ```
>
> 因为一张表的实际 row 只能按一种顺序存放，所以一张表通常只能有一个 clustered index；但可以有很多 secondary indexes。`clustered` 这个词强调的是“数据行和索引 key 聚在同一个物理结构里”，不是说它是分布式系统里的 cluster。
...
> [!NOTE] Wenbo 注
> 这里的关键点是：`in-memory database` 快，不只是因为“数据在 RAM 里”。如果一个普通的 disk-based database 的 working set 全部能放进内存，操作系统的 page cache 也会把常用磁盘页缓存到 RAM 中；很多 read 请求其实也不会真的碰磁盘。
>
> 真正的差别在于数据结构和编码方式。disk-based storage engine 通常要把数据组织成适合磁盘的格式，例如固定大小的 page、B-tree page、WAL、heap file、SSTable block、压缩块、序列化后的 row/document 等。即使这些 page 已经被 OS cache 放在内存里，数据库读取时仍然可能要做很多额外工作：解析 page 格式、解码 row、处理 slot/page header、从磁盘布局恢复成内存对象、维护 buffer pool 元数据等。
>
> `in-memory database` 可以直接使用更接近程序运行时的数据结构，例如 hash table、tree、skip list、linked list、set、priority queue。查询时可以直接沿着指针或数组访问对象，少了“把内存对象编码成磁盘格式、再从磁盘格式解码回内存对象”的来回转换。
>
> 可以粗略对比成这样：
>
> ```text
> disk-based database, 即使命中 OS cache:
> query -> 找到 cached page -> 解析 page/block -> 解码 row -> 返回结果
>
> in-memory database:
> query -> 访问内存里的 hash table / tree / set -> 返回结果
> ```
>
> 所以这段不是说 disk 不重要，而是说当数据已经都在内存里时，瓶颈会从 disk I/O 转移到 CPU、encoding/decoding、page layout、buffer management 这些开销。`in-memory database` 的优势主要来自“存储格式可以围绕内存访问来设计”，而不是单纯来自“读不到磁盘”。
...
<a id="fig_column_store"></a>

图 4-7. 按列而不是按行存储关系数据。

<img src="../../static/fig/ddia_0407.png" alt="图 4-7. 按列而不是按行存储关系数据。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
...
<a id="fig_bitmap_index"></a>

图 4-8. 单列的压缩、位图索引存储。

<img src="../../static/fig/ddia_0408.png" alt="图 4-8. 单列的压缩、位图索引存储。" style="display: block; margin: 1rem auto; width: 100%; max-width: 720px;" />
