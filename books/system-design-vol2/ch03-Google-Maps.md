...
<figure>
<img src="images/ch03-fig01-latitude-and-longitude-source-3.jpg" alt="" width="600">
<figcaption><em>Figure 1 Latitude and Longitude (source: [3])</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig02-map-projections-source-wikipedia-4-5-6-7.jpg" alt="A map of the world  Description automatically generated" width="600">
<figcaption><em>Figure 2 Map projections (source: Wikipedia [4] [5] [6] [7])</em></figcaption>
</figure>
...
Let’s assume the initial flattened surface is of size 20,000 km x 10,000 km. After the first divide, we would have 4 grids of size 10,000 km x 5,000 km. We represent them as 00, 01, 10, and 11 as shown in Figure 3. We further divide each grid into 4 grids and use the same naming strategy. Each sub-grid is now of size 5,000 km x 2,500 km. We recursively divide the grids until each grid reaches a certain size threshold.

<figure>
<img src="images/ch03-fig03-geohashing.jpg" alt="A map of the world  Description automatically generated" width="600">
<figcaption><em>Figure 3 Geohashing</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig04-graph.jpg" alt="A map with red circles  Description automatically generated" width="600">
<figcaption><em>Figure 4 Graph</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig05-routing-tiles.jpg" alt="" width="600">
<figcaption><em>Figure 5 Routing tiles</em></figcaption>
</figure>

> ReminderIn Figure 5, we call these grids routing tiles. Routing tiles are similar to map tiles in that both are grids covering certain geographical areas. Map tiles are PNG images, while routing tiles are binary files of road data for the area covered by the tiles.
...
<figure>
<img src="images/ch03-fig06-routing-tiles-of-varying-sizes.jpg" alt="" width="600">
<figcaption><em>Figure 6 Routing tiles of varying sizes</em></figcaption>
</figure>
...
Now we can analyze the server throughput for navigation requests. Let’s assume we have 1 billion DAU, and each user on average uses navigation 5 times for a total of 35 minutes per week. This translates to 35 billion minutes per week or 5 billion minutes per day. Navigation QPS: 1 billion x 5 / 7 / 105 = ~7,200
...
<figure>
<img src="images/ch03-fig07-high-level-design.jpg" alt="A diagram of a cloud service  Description automatically generated" width="600">
<figcaption><em>Figure 7 High-level design</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig09-batch-requests.jpg" alt="A diagram of a client  Description automatically generated" width="600">
<figcaption><em>Figure 9 Batch requests</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig10-cdn.jpg" alt="A diagram of a cloud  Description automatically generated" width="600">
<figcaption><em>Figure 10 CDN</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig11-without-cdn-vs-with-cdn.jpg" alt="A diagram of a computer network  Description automatically generated" width="600">
<figcaption><em>Figure 11 Without CDN vs with CDN</em></figcaption>
</figure>
...
> Data usageLet’s assume a user moves at 30 km/h, and at a zoom level where each image covers a block of 200m x 200m (a block can be represented by a 256-pixel by 256-pixel image and the average image size is 100KB). For an area of 1km x 1km, we need 25 images or 2.5 MB (25 x 100KB) of data. Therefore, if the speed is 30 km/h, we need 75MB (30 x 2.5MB) of data per hour or 1.25 MB of data per minute.

*Table 2 Data usage*

Next, we estimate the CDN data usage. At our scale, the cost is an important factor to consider.
> Traffic through CDNAs mentioned earlier, we serve 5 billion minutes of navigation per day. This translates to5 billion x 1.25 MB = 6.25 billion MB per day. Hence, we serve 62,500 MB (6.25 billion / 105 seconds in a day) of map data per second. With a CDN, these map images are going to be served from the POPs all over the world. Let’s assume there are 200 POPs. Each POP would only need to serve a few hundred MBs (62,500 / 200) per second.
...
Since we encode the grids in geohash, and there is one unique geohash per grid, computationally it is very efficient to go from the client’s location (in latitude and longitude) and zoom level to the geohash, for the map tile. This calculation can be done on the client. A geohash is a base-32 string that uniquely identifies the position and dimension of a grid. For example, the grid at zoom level 7, where the Google headquarters is located, is 9q9heb. By including the geohash in the URL, the client can fetch any static image tile from the CDN. For example, the URL for the image tile above could look like this: https://cdn.map-provider.com/tiles/9q9heb.png

Refer to Chapter 1 Proximity Service for a more detailed discussion of geohash encoding.
...
<figure>
<img src="images/ch03-fig12-map-rendering.jpg" alt="A diagram of a cloud  Description automatically generated" width="600">
<figcaption><em>Figure 12 Map rendering</em></figcaption>
</figure>
...
We will go into more detail on the pre-computed map tiles in the design deep dive.
...
Where should the routing tile processing service store these tiles? Most graph data is represented as adjacency lists [12] in memory. There are too many tiles to keep the entire set of adjacency lists in memory. If we could keep everything in memory, there would be no need for tiling in the first place. We could store the nodes and edges as rows in a database, but we would only be using the database as storage, and it seems an expensive way to store bits of data. We also don’t need any database features for routing tiles.

The more efficient way to store these tiles is in object storage like S3 and cache it aggressively on the routing service that uses those tiles. There are many high-performance software packages we could use to serialize the adjacency lists to a binary file. We could organize these tiles by their geohashes in the object storage. This provides a fast lookup mechanism to locate a tile by lat/lng pair.
...
> user_idtimestampuser_modedriving_modelocation1011635740977activedriving(20.0, 30.5)
...
<figure>
<img src="images/ch03-fig13-precomputed-tiles.jpg" alt="" width="600">
<figcaption><em>Figure 13 Precomputed tiles</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig14-user-location-database.jpg" alt="A diagram of a load balancer  Description automatically generated" width="600">
<figcaption><em>Figure 14 User location database</em></figcaption>
</figure>
...
> key (user_id)timestamplatlonguser_modenavigation_mode5113205300021.989.8activedriving
...
<figure>
<img src="images/ch03-fig15-location-data-is-used-by-other-services.jpg" alt="A diagram of a process  Description automatically generated" width="600">
<figcaption><em>Figure 15 Location data is used by other services</em></figcaption>
</figure>
...
In this section, we dive deep into pre-computed map tiles and map rendering optimization. They are primarily inspired by the work of Google Design [3].
...
<figure>
<img src="images/ch03-fig17-navigation-service.jpg" alt="A diagram of a service  Description automatically generated" width="600">
<figcaption><em>Figure 17 Navigation service</em></figcaption>
</figure>
...
*Figure 18 (based on [14]) gives a conceptual overview of the tiles used in the graph traversal.*

<figure>
<img src="images/ch03-fig18-graph-traversal.jpg" alt="A map with a red point and blue line  Description automatically generated" width="600">
<figcaption><em>Figure 18 Graph traversal</em></figcaption>
</figure>
...
Let’s start with a naive solution. In Figure 19, user_1’s navigation route is represented by routing tiles s_1, s_2, s_3, …, s_7.

<figure>
<img src="images/ch03-fig19-navigation-route.jpg" alt="A white rectangular object with black text  Description automatically generated" width="600">
<figcaption><em>Figure 19 Navigation route</em></figcaption>
</figure>
...
Let’s say there is a traffic incident in routing tile 2 (s_2). To figure out which users are affected, we scan through each row and check if routing tile 2 is in our list of routing tiles (see example below).
...
Assume the number of rows in the table is n and the average length of the navigation route is m. The time complexity to find all users affected by the traffic change is O (n x m).
...
<figure>
<img src="images/ch03-fig20-build-routing-tiles.jpg" alt="A diagram of a router  Description automatically generated" width="600">
<figcaption><em>Figure 20 Build routing tiles</em></figcaption>
</figure>
...
<figure>
<img src="images/ch03-fig21-final-design.jpg" alt="" width="600">
<figcaption><em>Figure 21 Final design</em></figcaption>
</figure>
